...
<tbody>
  <tr><td><code>build-wheels</code></td><td>Main entry point: Fromager resolve → build → auditwheel → SBOM patch</td></tr>
...
flowchart LR
    ...
    idx --> scans["Scans (parallel):<br>Clair | Snyk | Coverity<br>ShellCheck | Unicode<br>ClamAV | RPM sig"]
